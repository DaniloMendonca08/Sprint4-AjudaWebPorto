...
def verifica_cpf(conn, cpf):
    #função para verificar na tabela dos clientes se o cpf ja está cadastrado
    read_ist = conn.cursor()
...
def cadastro(conn, create_ist, update_ist):
    #função de cadastro e login que sao pedidos dados para ser inseridos na tabela 
    global data
    data['cpf'] = None  
    cpf_cliente = input('CPF: ')
    data['cpf'] = cpf_cliente
    existing_cpf = verifica_cpf(conn, cpf_cliente)
...
def sinistro(data, create_ist, conn, update_ist,cpf_cliente = None):
    ...
    cliente_data = verifica_cpf(conn, cpf_cliente)
...
def main_menu():
    ...
    while True:
        ...
        if y == 1:
            ...
            menuSinistro(data, create_ist, conn, delete_ist, read_ist, update_ist, placa_sinistro) 
        else:
